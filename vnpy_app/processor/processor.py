...
class Processor(ABC):
    def fit(self, df: pd.DataFrame):
        ...
        ----------
        df : pd.DataFrame
            When we fit and process data with processor one by one. The fit function relies on the output of previous
            processor, i.e. `df`.
    ...
    def readonly(self) -> bool:
        ...
        Knowning the readonly information is helpful to the Handler to avoid unnecessary copy
        """
        return False
...
class CSZScoreNorm(Processor):
    ...
    def __call__(self, df):
        ...
        df[cols] = df[cols].groupby("datetime", sort=False).apply(lambda x: (x - x.mean()).div(x.std())).values
        return df
